...
export function XPProgress({ className }) {
  const { currentLevel, nextLevel, currentXP, requiredXP } = XP_DATA;
  
  return (
    <div className={cn("bg-[#1A1B1E] rounded-2xl p-2 border border-gray-800/50", className)}>
      <div className="flex items-center gap-2">
        <div className="relative w-10 h-10 flex items-center justify-center">
          ...
          />
          <span className="text-white font-bold text-xl relative z-10 -mt-0.5">{currentLevel}</span>
        </div>
        <div className="flex-1">
          <div className="flex justify-between items-center mb-0.5">
            <span className="text-[#FFD32A] font-bold text-[10px]">LVL {currentLevel}</span>
            <span className="text-[#FFD32A] font-bold text-[10px]">LVL {nextLevel}</span>
          </div>
      ...
      <p className="text-[#FFD32A] text-[8px] text-center mt-1">
